...
export const ImageWithTextTemplate: React.FC<ImageWithTextProps> = ({ imagetext }) => {
  return (
    <div className='ImageWithTextTemplate py-10'>
      <h1 className='mb-6 md:text-[40px] text-[30px] capitalize lg:text-[46px] xl:text-[46px] text-center text-tertiary font-medium'>{imagetext.title}</h1>
      <GridBox columns={2} gap={10} className='py-5 items-center xl:space-y-0 lg:space-y-0 md:space-y-0 space-y-5'>
        <GridBox.GridItem columnMerge={1}>
          <Image className='border-neutral-800 rounded-[0px_50px_0px_50px] border-[15px] border-solid' src={imagetext.image} alt={imagetext.title} width={600} height={600} />
        </GridBox.GridItem>
        <GridBox.GridItem columnMerge={1}>
